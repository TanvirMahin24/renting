...
const PasswordChangeForm = ({ modals, updatePasswordAction }) => {
  ...
  const onSubmitHandeler = async (values) => {
    setLoading(true);
    let check = await updatePasswordAction(values);
    if (check === true) {
      setTimeout(() => {
        setLoading(false);
        modals.closeAll();
...
export default connect(null, { updatePasswordAction })(PasswordChangeForm);
